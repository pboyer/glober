 ...
 * animation.
 * 
 * The Scene3ds class constructors takes 3ds-file data as input, parses the data
 * and builds a memory image of the relevant 3D vector animation data. <br>
 * <br>
...
public class Scene3ds {
	/**
	 * Decode level constant. Decode all information. Unused parameters as hex
	 * bytes.
	 */
	...
	// Array of cameras
	private final Vector<Camera3ds> mCamera = new Vector<Camera3ds>(5, 5);

	// Array of materials
	private final Vector<Material3ds> mMaterial = new Vector<Material3ds>(5, 5);

	// Array of lights
	private final Vector<Light3ds> mLight = new Vector<Light3ds>(5, 5);
	 ...
	 * 
	 * @param i
	 *            index into camera array [0 ... cameras()-1]
	 * @return the specified camera
	 */
	public Camera3ds camera(int i) {
		return mCamera.elementAt(i);
	}
	 ...
	 * 
	 * @param i
	 *            index into material array [0 ... materials()-1]
	 * @return the specified material
	 */
	public Material3ds material(int i) {
		return mMaterial.elementAt(i);
	}
	 ...
	 * 
	 * @param i
	 *            index into light array [0 ... lights()-1]
	 * @return the specified light
	 */
	public Light3ds light(int i) {
		return mLight.elementAt(i);
	}
	...
	/**
	 * Construct (and decode) a Scene3ds object from a 3ds-file RAM image. The
	 * whole 3ds-file must be loaded beforhand into a <code>byte[]</code> array
	...
	/**
	 * Construct a Scene3ds object from a 3ds-file RAM image. The whole 3ds-file
	 * must be loaded beforhand into a <code>byte[]</code> array and passed as
	 * the <code>file_image</code> parameter.
	 * 
	 * @param file_image
	 *            file image of whole .3ds file
	 * @throws Exception3ds
	 ...
	 * 3ds-file data. The InputStream must contain exactly the 3ds-file data,
	...
	private int readUnsignedShort() throws Exception3ds {
		...
		int val = (mFileData[mFilePos] & 0xff)
				| ((mFileData[mFilePos + 1] & 0xff) << 8);
		mFilePos += 2;
		return val;
	}
	...
	private int readInt() throws Exception3ds {
		...
		int val = (mFileData[mFilePos] & 0xff)
				| ((mFileData[mFilePos + 1] & 0xff) << 8)
				| ((mFileData[mFilePos + 2] & 0xff) << 16)
				| (mFileData[mFilePos + 3] << 24);
		mFilePos += 4;
		return val;
	}
	...
	private void read_MAT_ENTRY(int chunk_len) throws Exception3ds {
		...
		while (filePos() < chunk_end) {
			...
			case CHUNK_MAT_TRANSPARENCY:
				...
				mat._transparency = 1.0f - mat._transparency; // values come
																// form 3ds
																// inverted, so
																// we need to
																// invert it
	...
	private void readSpotChunk(Light3ds pLight, int chunk_len)
		...
		if (mDecode != null) {
			...
			mDecode.println("Hotspot: "
					+ Utils3ds.floatToString(pLight.mHotspot, 12));
			mDecode.println("Falloff: "
					+ Utils3ds.floatToString(pLight.mFalloff, 12));
		}
	...
	private void read_CAM_RANGES(Camera3ds cam) throws Exception3ds {
		...
		if (mDecode != null) {
			mDecode.enter();
			mDecode.println("Near plane:"
					+ Utils3ds.floatToString(cam.mNearPlane, 14));
			mDecode.println("Far plane: "
					+ Utils3ds.floatToString(cam.mFarPlane, 14));
			mDecode.leave();
		}
	}
	...
	private TexCoord3ds[] read_TEX_VERTS() throws Exception3ds {
		...
		if (mDecode != null) {
			...
			for (int i = 0; i < coords; i++) {
				mDecode.println(" " + Utils3ds.intToString(i, 4) + ":  "
						+ tc[i]);
			}
			mDecode.leave();
		}
	...
	private void read_MESH_TEXTURE_INFO(Mesh3ds mes) throws Exception3ds {
		...
		if (mDecode != null) {
			...
			mDecode.println("Texture U tiling: "
					+ Utils3ds.floatToString(mes.mTexUTile, 9));
			mDecode.println("Texture V tiling: "
					+ Utils3ds.floatToString(mes.mTexVTile, 9));
		}
	...
	private void read_SMOOTH_GROUP(Mesh3ds mes, int chunk_len)
		...
		if (mDecode != null) {
			...
			for (int i = 0; i < entrys; i++) {
				mDecode.println(Utils3ds.intToString(i, 4) + ": "
						+ Utils3ds.intToBinString(mes.mSmoothGroup[i], 32));
			}
			mDecode.leave();
		}
	...
	private void read_OBJECT_NODE_TAG(int chunk_len) throws Exception3ds {
		...
		while (filePos() < chunk_end) {
			...
			case CHUNK_NODE_HDR:
				...
				if (mDecode != null) {
					mDecode.enter();
					mDecode.println("Node flags: 0x"
							+ Utils3ds.intToHexString(mes.mNodeFlags, 8));
	...
	private void read_TARGET_NODE_TAG(int chunk_len) throws Exception3ds {
		...
		while (filePos() < chunk_end) {
			...
			case CHUNK_NODE_HDR:
				...
				if (mDecode != null) {
					mDecode.enter();
					mDecode.println("Target node flags: 0x"
							+ Utils3ds.intToHexString(cam.mTargetNodeFlags, 8));
					mDecode.println("Target parent node id: "
							+ cam.mTargetParentNodeId);
					mDecode.leave();
				}
				// BUG: Build hierarchy here...
				break;
	...
	private void read_CAMERA_NODE_TAG(int chunk_len) throws Exception3ds {
		...
		while (filePos() < chunk_end) {
			...
			case CHUNK_NODE_HDR:
				...
				if (mDecode != null) {
					mDecode.enter();
					mDecode.println("Position node flags: 0x"
							+ Utils3ds
									.intToHexString(cam.mPositionNodeFlags, 8));
					mDecode.println("Position parent node id: "
							+ cam.mPositionParentNodeId);
					mDecode.leave();
				}
				// BUG: Build hierarchy here...
				break;
	...
	private void readSplineParams(SplineKey3ds key) throws Exception3ds {
		...
		if (flags != 0) {
			if ((flags & 0x01) != 0) {
				...
				if (mDecode != null) {
					mDecode.println("    Tension:    "
							+ Utils3ds.floatToString(key.Tension, 7));
				}
			}
			if ((flags & 0x02) != 0) {
				key.Bias = readFloat();
				if (mDecode != null) {
					mDecode.println("    Bias:       "
							+ Utils3ds.floatToString(key.Bias, 7));
				}
			}
			if ((flags & 0x04) != 0) {
				key.Continuity = readFloat();
				if (mDecode != null) {
					mDecode.println("    Continuity: "
							+ Utils3ds.floatToString(key.Continuity, 7));
				}
			}
			if ((flags & 0x08) != 0) {
				key.EaseTo = readFloat();
				if (mDecode != null) {
					mDecode.println("    Ease to:    "
							+ Utils3ds.floatToString(key.EaseTo, 7));
				}
			}
			if ((flags & 0x10) != 0) {
				key.EaseFrom = readFloat();
				if (mDecode != null) {
					mDecode.println("    Ease from:  "
							+ Utils3ds.floatToString(key.EaseFrom, 7));
				}
			}
		}
	}
	...
	static final int CHUNK_COL_RGB = 0x0010,
			...
			CHUNK_LIT_SPOT_PROJECTOR = 0x4653, CHUNK_LIT_SPOT_RANGE = 0x4655,
			CHUNK_LIT_SPOT_ROLL = 0x4656, CHUNK_LIT_SPOT_ASPECT = 0x4657,
			CHUNK_LIT_RAY_BIAS = 0x4658, CHUNK_LIT_INNER_RANGE = 0x4659,
			CHUNK_LIT_OUTER_RANGE = 0x465A, CHUNK_LIT_MULTIPLIER = 0x465B,
			CHUNK_N_CAMERA = 0x4700, CHUNK_CAM_SEE_CONE = 0x4710,
